...
func (d *Debug) traceTransactionWithAllClauses(ctx context.Context, blockID meter.Bytes32, txIndex uint64) (interface{}, error) {
	...
	if se {
		d.logger.Debug("Tx has Script Engine clause, skip tracing", "se", se, "blockID", blockID, "txIndex", txIndex, "clauseIndex", 0)
		return nil, nil
	}
	...
	for txExec.HasNextClause() {
		d.logger.Debug("execute clause", "blockID", blockID, "txIndex", txIndex, "clauseIndex", clauseIndex)
...
func (d *Debug) execClause(ctx context.Context, blockID meter.Bytes32, txIndex uint64, clauseIndex uint64) (interface{}, error) {
	...
	if err != nil {
		d.logger.Error("error handle clause env", "err", err)
		return nil, err
	}
	gasUsed, output, err := txExec.NextClause()
	if err != nil {
		d.logger.Error("error next clause", "err", err)
		return nil, err
	}
...
func (d *Debug) convertTraceData(callTraceResult CallTraceResult, path []uint64, blockHash meter.Bytes32, blockNumber uint64, txHash meter.Bytes32, txIndex uint64) ([]*TraceData, error) {
	...
	if err != nil {
		d.logger.Error("parse address failed, set it empty", "err", err)
		from = meter.Address{}
	}
	...
	for index, call := range callTraceResult.Calls {
		// d.logger.Debug("convert subcall data: ", call, path, blockHash, blockNumber, txIndex)
		subdatas, err := d.convertTraceData(call, append(path, uint64(index)), blockHash, blockNumber, txHash, txIndex)
		if err != nil {
			d.logger.Error("Error happened during subdata query", "err", err)
		}
...
func (d *Debug) handleRerunTransaction(w http.ResponseWriter, req *http.Request) error {
	...
	if err != nil {
		d.logger.Error("error parsing txid", "err", err)
		return err
	}
	best := d.chain.BestBlock()
	txMeta, err := d.chain.GetTransactionMeta(txID, best.ID())
	if err != nil {
		d.logger.Error("error getting txmeta", "err", err)
		return err
	}
	d.logger.Debug("handle trace", "blockID", txMeta.BlockID, "txIndex", txMeta.Index, "clauseIndex", clauseIndex)
	res, err := d.execClause(req.Context(), txMeta.BlockID, txMeta.Index, uint64(clauseIndex))
	if err != nil {
		return err
	}
	d.logger.Debug("complete handle trace")
...
func (d *Debug) handleTraceTransaction(w http.ResponseWriter, req *http.Request) error {
	...
	d.logger.Debug("handle tracer", "blockID", blockID, "txIndex", txIndex, "clauseIndex", clauseIndex)
	if err != nil {
		return err
	}
	...
	d.logger.Debug("complete handle tracer")
	return utils.WriteJSON(w, res)
}
...
func (d *Debug) openEthTraceTransaction(ctx context.Context, txHash meter.Bytes32) ([]*TraceData, error) {
	...
	if err != nil {
		d.logger.Error("Error happened in GetTrunkTransaction", "err", err)
		return datas, err
	}
	_, err = tx.Signer()
	if err != nil {
		d.logger.Error("Error: could not get signer:", "err", err)
		return datas, err
	}
	d.logger.Debug("Openeth trace tx", "id", tx.ID(), "blockID:", meta.BlockID)
	blk, err := d.getBlock(meta.BlockID)
	if err != nil {
		d.logger.Error("Could not get block", "err", err)
		return datas, err
	}
	...
	d.logger.Debug("Completed openeth trace tx: ", "data entries", len(datas))
	return datas, nil
...
func (d *Debug) handleOpenEthTraceTransaction(w http.ResponseWriter, req *http.Request) error {
	...
	d.logger.Debug("handle trace transaction")
	...
	d.logger.Debug("complete handle trace tx with", "data entries", len(results))
...
func (d *Debug) handleOpenEthTraceBlock(w http.ResponseWriter, req *http.Request) error {
	...
	d.logger.Debug("handle trace block", "revision:", revision)
	if err != nil {
		d.logger.Debug("Error: could not parse reivision", err)
		return utils.BadRequest(errors.WithMessage(err, "could not parse revision"))
	}
	blk, err := d.getBlock(revision)
	if err != nil {
		d.logger.Error("Error: could not get block", "err", err)
	...
	d.logger.Debug("complete handle trace block with", "data entries", len(results))
	return utils.WriteJSON(w, results)
}
...
func (d *Debug) handleOpenEthTraceFilter(w http.ResponseWriter, req *http.Request) error {
	...
	if opt.FromBlock != "" {
		...
		if err != nil {
			d.logger.Error("Error: invalid fromBlock", "err", err)
	...
	if opt.ToBlock != "" {
		...
		if err != nil {
			d.logger.Error("Error: invalid toBlock", "err", err)
	...
	d.logger.Debug("handle trace filter", "fromBlock", fromBlockNum, "toBlock", toBlockNum)
	...
	d.logger.Debug("complete handle trace filter with ", "data entries", len(results))
	return utils.WriteJSON(w, results)
}
...
func (d *Debug) handleSupply(w http.ResponseWriter, req *http.Request) error {
	...
	d.logger.Debug("handle get meter info", "revision", revision)
	if err != nil {
		d.logger.Error("Error: could not parse reivision", "err", err)
		return utils.BadRequest(errors.WithMessage(err, "could not parse revision"))
	}
	blk, err := d.getBlock(revision)
	if err != nil {
