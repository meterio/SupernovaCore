...
func (p *Pacemaker) scheduleBroadcast(proposalMsg *block.PMProposalMessage, d time.Duration) {
	...
	if d <= 0 || d >= BroadcastTimeLimit {
		p.logger.Debug(fmt.Sprintf("schedule broadcast for %s(E:%d) with no delay", blk.CompactString(), proposalMsg.GetEpoch()))
		scheduleFunc()
	} else {
		p.logger.Debug(fmt.Sprintf("schedule broadcast for %s(E:%d) after %s", blk.CompactString(), proposalMsg.GetEpoch(), types.PrettyDuration(d)))
