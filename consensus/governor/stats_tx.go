	...
	for _, blk := range blocks {
		...
		for _, v := range violations {
			if v.Index < len(committeeInfo) {
				...
				blsPK, err := bls.PublicKeyFromBytes(blsPKBytes)
				if err != nil {
					break
				}
				sig1, err := bls.SignatureFromBytes(v.Signature1)
				if err != nil {
					break
				}
				sig2, err := bls.SignatureFromBytes(v.Signature2)
				if err != nil {
					break
				}
				sig1.Verify(blsPK, v.MsgHash[:])
				sig2.Verify(blsPK, v.MsgHash[:])
...
func combinePubKey(blsCommon *types.BlsCommon, ecdsaPub *ecdsa.PublicKey, blsPub bls.PublicKey) string {
	...
	blsPubB64 := b64.StdEncoding.EncodeToString(blsPub.Marshal())
